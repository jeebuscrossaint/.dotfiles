...
Item {
    ...
    readonly property HyprlandMonitor monitor: Compositor.isHyprland ? Hyprland.monitorFor(screen) : null
    ...
    property bool monitorIsFocused: (screen?.name === Compositor.focusedMonitorName)
                            ...
                            MouseArea {
                                ...
                                onPressed: {
                                    if (root.draggingTargetWorkspace === -1) {
                                        ...
                                        Compositor.switchWorkspace(workspace.workspaceValue)
                                    }
                                }
                            }
                    ...
                    MouseArea {
                        ...
                        onReleased: {
                            ...
                            if (targetWorkspace !== -1 && targetWorkspace !== windowData?.workspace.id) {
                                if (Compositor.isHyprland) Hyprland.dispatch(`movetoworkspacesilent ${targetWorkspace}, address:${window.windowData?.address}`)
                                ...
                                if (Compositor.isHyprland) Hyprland.dispatch(`movewindowpixel exact ${percentageX}% ${percentageY}%, address:${window.windowData?.address}`)
                            }
                        }
                        onClicked: (event) => {
                            ...
                            if (event.button === Qt.LeftButton) {
                                GlobalStates.overviewOpen = false
                                if (Compositor.isHyprland) Hyprland.dispatch(`focuswindow address:${windowData.address}`)
                                event.accepted = true
                            } else if (event.button === Qt.MiddleButton) {
                                if (Compositor.isHyprland) Hyprland.dispatch(`closewindow address:${windowData.address}`)
                                event.accepted = true
                            }
                        }
