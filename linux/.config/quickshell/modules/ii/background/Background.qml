...
Variants {
    ...
    PanelWindow {
        ...
        // Workspaces
        property HyprlandMonitor monitor: Compositor.isHyprland ? Hyprland.monitorFor(modelData) : null
        // relevantWindows: on Hyprland use the client list; on other compositors use ToplevelManager
        property list<var> relevantWindows: Compositor.isHyprland
            ? HyprlandData.windowList.filter(win => win.monitor == monitor?.id && win.workspace.id >= 0).sort((a, b) => a.workspace.id - b.workspace.id)
            : []
