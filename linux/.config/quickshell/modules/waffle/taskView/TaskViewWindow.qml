...
WMouseAreaButton {
    ...
    function closeWindow() {
        if (Compositor.isHyprland) Hyprland.dispatch(`closewindow address:${root.hyprlandClient?.address}`);
    }
    ...
    onClicked: event => {
        if (event.button === Qt.LeftButton) {
            ...
            if (Compositor.isHyprland) Hyprland.dispatch(`focuswindow address:${root.hyprlandClient?.address}`);
            GlobalStates.overviewOpen = false;
    ...
    WMenu {
        ...
        Action {
            ...
            onTriggered: {
                if (Compositor.isHyprland) Hyprland.dispatch(`pin address:${root.hyprlandClient?.address}`);
            }
        }
        Action {
            icon.name: "empty"
