...
Singleton {
    ...
    // Update the layout name when it changes (Hyprland)
    Connections {
        target: Hyprland
    ...
    // MangoWM: MangoData.keyboardLayout is the raw code (e.g. "us")
    // Map directly: both name and code are the same raw code from mmsg
    Connections {
        target: MangoData
        function onKeyboardLayoutChanged() {
            if (!Compositor.isMango) return;
            const code = MangoData.keyboardLayout;
            if (!code) return;
            root.currentLayoutCode = code;
            root.currentLayoutName = code;
            root.layoutCodes = [code];
        }
    }

    // niri: keyboard layout comes from XKB state, not niri IPC directly.
    // Use setxkbmap -query as a fallback to get current layout.
    Process {
        id: niriLayoutProc
        running: Compositor.isNiri
        command: ["sh", "-c", "setxkbmap -query 2>/dev/null | awk '/layout/ {print $2}'"]
        stdout: StdioCollector {
            id: niriLayoutCollector
            onStreamFinished: {
                const code = niriLayoutCollector.text.trim();
                if (code) {
                    root.currentLayoutCode = code;
                    root.currentLayoutName = code;
                    root.layoutCodes = code.split(",");
                }
            }
        }
    }
